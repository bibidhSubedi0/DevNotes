...
export default function ProjectNode({ id, data, selected }) {
  ...
  const handleCancel  = () => { setEditValue(data.label); setIsEditing(false); setValidationError(''); };
  const handleKeyDown = (e) => { if (e.key === 'Enter') { e.preventDefault(); handleSave(); } if (e.key === 'Escape') handleCancel(); };
  const handleChange  = (e) => { setEditValue(e.target.value); if (validationError) setValidationError(''); };

  // Short description preview shown on the circle
  const hasDesc   = Boolean(data.description?.trim());
  const shortDesc = data.description?.length > 38 ? data.description.slice(0, 36) + '…' : data.description;

  // Stage badge color
  const stageColors = {
    idea:         'text-neutral-400 bg-neutral-700/60',
    prototyping:  'text-amber-400   bg-amber-500/10',
  ...
  const stageClass = stageColors[data.stage] || stageColors.idea;

  return (
    <div className="relative group">
      {/* Validation error banner */}
      {validationError && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[99999] bg-red-500 text-white px-6 py-4 rounded-xl shadow-2xl border-4 border-white" style={{ minWidth: '300px' }}>
          <div className="flex items-center gap-3 text-lg font-bold">
            <AlertCircle size={28} />
            <div>
              <div className="text-xl">⚠ VALIDATION ERROR</div>
      ...
      <div className={`absolute inset-0 rounded-full transition-all duration-300
        ${selected ? 'bg-gradient-to-br from-purple-500/30 to-indigo-500/30 blur-xl scale-110'
          : 'bg-gradient-to-br from-purple-600/20 to-indigo-600/20 blur-lg scale-100 opacity-0 group-hover:opacity-100'}`} />

      {/* Main circle */}
      <div className={`relative w-48 h-48 bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-700
        rounded-full flex flex-col items-center justify-center border-4 transition-all duration-300
        shadow-[0_0_40px_rgba(168,85,247,0.4)]
        ${selected ? 'border-purple-300 scale-105' : 'border-purple-400/80 group-hover:border-purple-300 group-hover:scale-105'}
        ...
        <div className="p-3 bg-white/10 rounded-2xl mb-1.5 backdrop-blur-sm">
        ...
        {isEditing ? (
          <div className="px-4 w-full flex flex-col items-center gap-2" onClick={e => e.stopPropagation()}>
            <input type="text" value={editValue} onChange={handleChange} onKeyDown={handleKeyDown}
              autoFocus maxLength={50}
              className={`w-full bg-white/20 text-white text-base font-bold px-2 py-1 rounded border-4 focus:outline-none text-center
            ...
            <div className="flex gap-2 justify-center">
              <button onClick={handleSave}   className="px-3 py-2 bg-green-500 hover:bg-green-400 rounded-lg text-white font-bold transition-colors shadow-lg"><Check size={16} /></button>
              <button onClick={handleCancel} className="px-3 py-2 bg-red-500   hover:bg-red-400   rounded-lg text-white font-bold transition-colors shadow-lg"><X size={16} /></button>
            </div>
          </div>
        ) : (
          <>
            <div
              ...
              onDoubleClick={() => { setIsEditing(true); setEditValue(data.label); setValidationError(''); }}
            ...
            {/* Stage badge */}
            {data.stage ? (
              <span className={`text-[10px] font-bold uppercase tracking-widest mt-1 px-2 py-0.5 rounded-full ${stageClass}`}>
                {data.stage}
              </span>
            ) : (
              <div className="text-[10px] uppercase text-purple-100 font-semibold tracking-wider mt-1">Project Root</div>
            )}

            {/* Description preview */}
            {hasDesc && (
              <div className="text-[9px] text-purple-200/50 text-center px-3 mt-1.5 leading-tight line-clamp-2">
                {shortDesc}
              </div>
            )}
          </>
